...
// static void add_char_to_text_area(char c); // Removed - not used directly
...
void init_styles()
{
    ...
    lv_style_set_pad_ver(&style_input_cont, 8); // Adjust for vertical centering
...
void create_text_area(lv_obj_t *parent)
{
    ...
    lv_obj_set_size(area, SCREEN_WIDTH, TEXT_AREA_HEIGHT);
...
static void blob_key_event_cb(lv_event_t *e)
{
    ...
    lv_obj_t *key = static_cast<lv_obj_t*>(lv_event_get_target(e)); // Use static_cast
    if (!key) return; // Should not happen, but good practice
    ...
    if (code == LV_EVENT_PRESSING)
    {
        ...
        if (indev)
        {
            ...
            // Simple detection based on x coordinate relative to key center
            lv_coord_t key_x = lv_obj_get_x(key);
            lv_coord_t key_width = lv_obj_get_width(key);
            lv_coord_t touch_x_rel = last_touch_point.x - key_x;

            // Determine letter index (0: left, 1: center, 2: right)
            // Basic thresholding - adjust thresholds as needed
    ...
    else if (code == LV_EVENT_RELEASED || code == LV_EVENT_PRESS_LOST)
    {
        ...
        {
            // Add character to input buffer only on release
            if (code == LV_EVENT_RELEASED)
            {
                add_char_to_input(letters[active_blob_key_letter_index]);
            }
            // Reset visuals after a short delay to show the selected state briefly
            lv_timer_t *t = lv_timer_create([](lv_timer_t *timer)
                                            {
                                                lv_obj_t *target_key = (lv_obj_t *)timer->user_data;
                                                reset_blob_key_visuals(target_key);
                                                lv_timer_delete(timer); // Delete one-shot timer
                                            },
                                            100, key); // 100ms delay
...
static void update_blob_key_visuals(lv_obj_t *key, int letter_index, bool pressed)
{
    ...
    for (uint32_t i = 0; i < lv_obj_get_child_count(key); i++)
    {
        ...
        if (lv_obj_check_type(child, &lv_label_class)) // Check if it's a label
        {
            if (i == letter_index)
            {                                                // This label corresponds to the selected index
...
static void update_input_display()
{
    ...
    lv_coord_t label_w = lv_obj_get_width(input_text_label);
    lv_obj_align(input_cursor, LV_ALIGN_LEFT_MID, label_w + 1, 0); // Position cursor right after text
}

static void update_text_area_display()
{
    // This function assumes text_content_label already holds the correct text
    ...
    // Get the position of the last character (or start if empty)
    lv_point_t pos;
    const lv_font_t *font = lv_obj_get_style_text_font(text_content_label, 0);
    const char * txt = lv_label_get_text(text_content_label);
    lv_label_get_letter_pos(text_content_label, strlen(txt), &pos); // Use lv_label_get_letter_pos and strlen

    // Adjust position slightly for better alignment
    pos.y += (lv_font_get_line_height(font) - lv_obj_get_height(text_cursor)) / 2;
...
// Removed unused function add_char_to_text_area
// static void add_char_to_text_area(char c)
// {
//     // This logic is now handled within accept_input
// }
...
static void accept_input()
{
    if (strlen(input_buffer) > 0)
    {
        ...
        // Allocate a new buffer large enough for both strings + null terminator
        // This is inefficient for long text, consider lv_label_ins_text or lv_textarea for real apps
        ...
        if (new_text) {
            ...
            free(new_text); // Free the allocated buffer
        } else {
             log_e("Failed to allocate memory for accept_input");
             // Handle error: maybe just clear input?
        }

        clear_input();
        update_text_area_display(); // Update cursor position
    }
}
...
void setup()
{
    ...
    smartdisplay_init();
    // Optional: Set adaptive brightness
    // smartdisplay_lcd_set_brightness_cb(smartdisplay_lcd_adaptive_brightness_cds, 100);

    __attribute__((unused)) auto disp = lv_disp_get_default();
    // Ensure correct rotation if needed, default is usually fine for 480x320 boards
    // lv_disp_set_rotation(disp, LV_DISP_ROT_LANDSCAPE); // Or LV_DISP_ROT_LANDSCAPE_INVERTED
...
void loop()
{
    ...
    // but lv_timer_handler should yield if configured correctly.
    // delay(5);
}